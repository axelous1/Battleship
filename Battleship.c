...
int playerMove(char playerView[10][10], char enemy[10][10]) {
    int x, y;
    int result;
    while (1) {
...
void getNextHuntShot(char player[10][10], int* x, int* y) {
    ...
    int lastX = hunt.hitPoints[hunt.hitCount - 1][0];
    int lastY = hunt.hitPoints[hunt.hitCount - 1][1];
    int nx = lastX + hunt.dirX;
    int ny = lastY + hunt.dirY;
...
int main(int argc, char* argv[]) {
    ...
    while (!gameOver) {
        if (turn == 1) {
            ...
            int hit = playerMove(playerView, enemyField);
